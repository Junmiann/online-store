...
from db import *
from utils import *
from models.userProfileService import UserProfileService
...
def user_profile():
    ...
    if user is None:
        ...
        user_orders = UserProfileService.get_user_orders(con, user[0], order_id)
...
def user_order_details(order_id):
    user = session.get("user")
    user_orders_details = UserProfileService.get_user_order_details(con, order_id)
    user_order_products = UserProfileService.get_user_order_products(con, order_id)
